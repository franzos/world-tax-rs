...
use crate::{errors::DatabaseError, types::{TaxRate, TradeAgreement}};
...
impl TaxDatabase {
    ...
    pub fn get_rate(&self, country: &str, region: Option<&str>, vat_rate: Option<&VatRate>) -> Result<Vec<TaxRate>, DatabaseError> {
        ...
        match country_data.tax_type {
            ...
            TaxSystemType::Vat => {
                self.handle_vat_rates(country_data, vat_rate, &mut rates)?
            },
    ...
    fn handle_vat_rates(&self, country: &Country, vat_rate: Option<&VatRate>, rates: &mut Vec<TaxRate>) -> Result<(), DatabaseError> {
    ...
    fn handle_gst_rates(&self, country: &Country, region: Option<&str>, rates: &mut Vec<TaxRate>) -> Result<(), DatabaseError> {
        if let Some(region_code) = region {
